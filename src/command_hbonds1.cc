...
Data bb_bb_frame_counter=0;
Data bb_sc_frame_counter=0;
Data sc_sc_frame_counter=0;


CommandHbonds1::CommandHbonds1() 
	:Command("IHBONDS")
{
}

void	CommandHbonds1::SubRun(Team *team1,Team *team2,Tex &myTex)
{
	...
	if(team1==NULL || team2==NULL ) printf("fatal error \n");
	
	for(int i=0;i<Donor.size();i++)
	{
		if(team1 == NULL || team1->find(table[Donor[i]-1],donor_chain))
		{
	...
	{
		if(team2==NULL ||  team2->find(table[Acceptor[i]-1],acceptor_chain))
		{
...
void	CommandHbonds1::Run()
{
	...
	Log("Critical angle      = "+printDistance(backbone_critical_angle));
	Log("\n");
	string ff="";
	ff="#Donor";
	for(int ll=0;ll<18-6;ll++) ff+=" ";
	ff+="Acceptor";
	for(int ll=0;ll<18-8;ll++) ff+=" ";
	Log(ff+" "+printString("Exist",PERCENT_WIDTH));

	SubRun(team1,team2,myTex);
	SubRun(team2,team1,myTex);
